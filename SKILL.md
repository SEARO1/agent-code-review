...
For whole-change reviews of AI-agent-authored code, the companion [agent-code-review](agent-code-review/SKILL.md) supplies the requirements, integration, and test-integrity workflow. When called from that review, contribute complexity findings without restarting its workflow. Complexity-only requests stay within the scope below.
